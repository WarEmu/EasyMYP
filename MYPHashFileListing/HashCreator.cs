...
namespace MYPHashFileListing
{
    class HashCreator
    {
        ...
        public void IntegrateOldHashList()
        {
            ...
            hasher.IntegreateOldHashList(file);
        }
        ...
        public void NewFile(object sender, MYPWorker.MYPFileTableEventArgs e)
        {
            ...
            hasher.UpdateHash(newfile.ph, newfile.sh, newfile.filename);
        }
        ...
        {
            SortedList<uint, PrimaryHashEntry> xmlHashList = new SortedList<uint, PrimaryHashEntry>();
            ...
            if (File.Exists("hashes.xml"))
            {
                ...
                {
                    xmlHashList.Add(tmpList[i].ph, tmpList[i]);
                }

                xmlStream.Close();

                for (int i = 0; i < xmlHashList.Count; i++)
                {
                    for (int j = 0; j < xmlHashList.Values[i].shList.Count; j++)
                    {
                        hasher.UpdateHash(xmlHashList.Values[i].ph
                            , xmlHashList.Values[i].shList[j].sh
                            , xmlHashList.Values[i].shList[j].filename);
                    }
                }
        ...
        public void ParseCSV()
        {
            ...
            FileStream csv_stream = new FileStream("CSV/art.csv", FileMode.Open);
            //5: directory
            //4: file name
            StreamReader reader = new StreamReader(csv_stream);
            WarHasher warhash = new WarHasher();
            string line;
            #region art.csv
            ...
            while ((line = reader.ReadLine()) != null)
            {
                string directory = line.Split(';')[5];
                for (int i = 0; i < 15; i++)//34
                {
                    string filename = line.Split(';')[7 + 2 * i];
                    string cur_str = directory + '\\' + filename;
                    warhash.Hash(cur_str, 0xDEADBEEF);
                    ...
                    directory = directory.Replace("art_source", "assetdb");

                    cur_str = directory + '\\' + filename;
                    warhash.Hash(cur_str, 0xDEADBEEF);
                    ...
                    directory = directory.Replace("assetdb", "art");
            ...
            csv_stream = new FileStream("CSV/nifs.csv", FileMode.Open);
            //5: directory
            //4: file name
            reader = new StreamReader(csv_stream);
            warhash = new WarHasher();

            #region nifs.csv

            while ((line = reader.ReadLine()) != null)
            {
                string directory = line.Split(';')[3];
                for (int i = 0; i < 1; i++)//34
                {
                    string filename = line.Split(';')[2 + 2 * i];
                    string cur_str = directory + '\\' + filename;
                    warhash.Hash(cur_str, 0xDEADBEEF);
            ...
            csv_stream = new FileStream("CSV/lightmap.csv", FileMode.Open);//zones.csv
            //5: directory
            //4: file name
            reader = new StreamReader(csv_stream);
            warhash = new WarHasher();
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                for (int i = 0; i < 1; i++)//34
                {
                    string filename = line.Split(';')[4 + 2 * i];
                    string cur_str = directory + '\\' + filename;
                    warhash.Hash(cur_str, 0xDEADBEEF);
            ...
            for (int i = 0; i < hasher.HashList.Count; i++)
            {
                ...
                newfile.ph = (uint)((hasher.HashList.Keys[i] >> 32) & 0xFFFFFFFF);
                newfile.sh = (uint)(hasher.HashList.Keys[i] & 0xFFFFFFFF);
                newfile.filename = hasher.HashList.Values[i];
                mlist.Add(newfile);
            }
